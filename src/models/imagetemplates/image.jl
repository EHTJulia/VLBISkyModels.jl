...
$(FIELDS)
"""
struct LogSpiral{T <: Number} <: AbstractImageTemplate
...
radialextent(d::LogSpiral{T}) where {T} = exp(d.κ * d.δϕ)
...
@inline intensity_point(c::Constant, p) = inv(getparam(c, :scale, p))^2
...
radialextent(d::GaussDisk{T}) where {T} = one(T) + 3 * d.α

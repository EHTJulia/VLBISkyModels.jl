@testset "image modifiers" begin
    ...
    @testset "Rotate invariant" begin
        ...
        @test isapprox(img2, img, rtol=1e-2)

        pimg2 = rotated(pimg, π / 4)
        @test isapprox(stokes(pimg2, :U), stokes(pimg, :Q), rtol=1e-2)
        @test isapprox(stokes(pimg2, :Q), stokes(pimg, :U), atol=1e-6)
        @test isapprox(stokes(pimg2, :V), stokes(pimg, :V), rtol=1e-2)
        @test isapprox(stokes(pimg2, :I), img2, rtol=1e-2)
    end
    ...
    @testset "center image" begin
        ...
        @test isapprox(img, img2_c, rtol=1e-2)

        pimg2_c = center_image(pimg2)
        @test isapprox(pimg, pimg2_c, rtol=1e-2)

        imgs = [copy(img2) for _ in 1:10]
        cmimg = centroid_mean(imgs)
        @test isapprox(img, cmimg, rtol=1e-2)
    end
end

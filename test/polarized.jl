...
@testset "Polarized Semi Analytic" begin
    ...
    @test isapprox(vff, vnf, atol = 1.0e-6)
    @test isapprox(vff, vdf, atol = 1.0e-6)
    @test isapprox(vnf, vdf, atol = 1.0e-6)
end
...
@testset "Polarized Modified" begin
    ...
    s = map(length, dims(g))
    m0 = PolarizedModel(
        ExtendedRing(2.0), 0.1 * Gaussian(), 0.1 * Gaussian(),
        0.1 * Gaussian()
    )
...
@testset "Polarized NonAnalytic" begin
    ...
    @test isapprox(maximum(norm, v .- van), 0.0, atol = 1.0e-5)
end

@testset "Polarized All Mod" begin
    m1 = PolarizedModel(Gaussian(), 0.1 * Gaussian(), 0.1 * Gaussian(), 0.1 * Gaussian())
    m2 = PolarizedModel(
        ExtendedRing(8.0), shifted(Disk(), 0.1, 1.0), ZeroModel(),
        ZeroModel()
    )
...
@testset "Rotation" begin
    ...
    @test all(==(1), isapprox.(stokes(img2, :Q), 0.0, atol = 1.0e-16))
    ...
    @test all(==(1), isapprox.(stokes(img3, :U), 0.0, atol = 1.0e-16))
    ...
    @test all(==(1), isapprox.(stokes(img4, :Q), 0.0, atol = 1.0e-16))
...
@testset "ContinuousImage" begin
    ...
    @test all(==(1), isapprox.(stokes(rimg1, :Q), 0.0, atol = 1.0e-16))
    ...
    @test isapprox(maximum(norm, vnf .- vdf), 0.0, atol = 1.0e-6)
    @test isapprox(maximum(norm, vff .- vdf), 0.0, atol = 1.0e-3)
end
...
@testset "PoincareSphere2Map" begin
    ...
    gfour = FourierDualDomain(
        g, UnstructuredDomain((U = randn(32) / 5, V = randn(32) / 5)),
        NFFTAlg()
    )

@testset "Moments" begin
    ...
    intensitymap!(img, m1)
    @test isapprox(centroid(img)[1], 0.0, atol = 1.0e-5)
    @test isapprox(centroid(img)[2], 0.0, atol = 1.0e-5)

    I = second_moment(img)
    I2 = second_moment(img; center = false)
    @test isapprox(I, [1.0 0.0; 0.0 1.0], atol = 1.0e-5)
    @test I ≈ I2

    m2 = shifted(m1, 1.0, 1.0)
    intensitymap!(img, m2)
    @test isapprox(centroid(img)[1], 1.0, atol = 1.0e-5)
    @test isapprox(centroid(img)[2], 1.0, atol = 1.0e-5)
    @test isapprox(second_moment(img), I, atol = 1.0e-5)
    ...
    @test isapprox(I3, [4.0 0.0; 0.0 1.0], atol = 1.0e-5)
end
...
@testset "Primitive models" begin
    ...
    @testset "MRing1" begin
        ...
        testmodel(m, 1024, 1.0e-3)
        @inferred VLBISkyModels.visibility(m.m1, (U = 0.0, V = 0.0))
        @inferred VLBISkyModels.intensity_point(m.m1, (X = 0.0, Y = 0.0))

        testmodel(MRing(0.5, 0.0), 3200, 1.0e-2, 0.25)
    ...
    @testset "Crescent" begin
        ...
        foo(x) = sum(
            abs2,
            VLBISkyModels.visibilitymap_analytic(
                Crescent(x[1], x[2], x[3], x[4]),
                g
            )
        )
        x = rand(4)
        foo(x)
        testgrad(foo, x; atol = 0, rtol = 1.0e-5)
    end
...
@testset "ModelImage" begin
    ...
    @test isapprox(maximum(parent(img) - img2), 0.0, atol = 1.0e-8)
    @test isapprox(maximum(parent(img) - img2), 0.0, atol = 1.0e-8)
end
...
@testset "CompositeModels" begin
    ...
    @testset "Convolved models" begin
        ...
        @test intensitymap(stretched(m1, sqrt(2)), gfour) ≈
            intensitymap(convolved(m1, m1), gfour) rtol=1e-6
    ...
    @testset "Convolved Non-analytic" begin
        ...
        @test isapprox(parent(img1), parent(img2), rtol = 1.0e-8)
    end
...
@testset "Multicomponent" begin
    fname = Downloads.download(
        "http://www.cv.nrao.edu/2cmVLBA/data/0003-066/2012_03_04/0003-066.u.2012_03_04.icn.fits.gz",
...
@testset "PolarizedModel" begin
    ...
    @test isapprox(sum(abs, (stokes(pimg1, :I) .- stokes(pimg2, :I))), 0.0, atol = 1.0e-12)
    @test isapprox(sum(abs, (stokes(pimg1, :Q) .- stokes(pimg2, :Q))), 0.0, atol = 1.0e-12)
    @test isapprox(sum(abs, (stokes(pimg1, :U) .- stokes(pimg2, :U))), 0.0, atol = 1.0e-12)
    @test isapprox(sum(abs, (stokes(pimg1, :V) .- stokes(pimg2, :V))), 0.0, atol = 1.0e-12)
end
...
@testset "methods " begin
    u1cp = 10.0e9 * rand(100) .- 5.0e9
    v1cp = 10.0e9 * rand(100) .- 5.0e9
    u2cp = 10.0e9 * rand(100) .- 5.0e9
    v2cp = 10.0e9 * rand(100) .- 5.0e9
    u3cp = -u1cp - u2cp
    v3cp = -v1cp - v2cp

    u1lc = 10.0e9 * rand(100) .- 5.0e9
    v1lc = 10.0e9 * rand(100) .- 5.0e9
    u2lc = 10.0e9 * rand(100) .- 5.0e9
    v2lc = 10.0e9 * rand(100) .- 5.0e9
    u3lc = 10.0e9 * rand(100) .- 5.0e9
    v3lc = 10.0e9 * rand(100) .- 5.0e9
    u4lc = -u1lc - u2lc - u3lc
    v4lc = -v1lc - v2lc - v3lc
